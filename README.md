...
This clone of ultalytics/ultralytics has been modified adding DetectSeverity which predicts box+severity for each object. 

Similar to OBB, DetectSev has 5 regressor output per object detection. 

However, the 5th regressor "severity" is unrelated geometrically so the OBB classes could not be used without running into issues to do with xywhr2xyxy conversions.

WIP - not yet functional.  

TODO:

* ultralytics.utils.loss.v8DetectionLoss.preprocess may need modifying to initialise to a different number of zeros at line 
* ultralytics.models.utils.ops.get_cdn_group lin 230 padding_bbox = torch.zeros(bs, num_dn, 4, device=gt_bbox.device) probably needs 4 changing to 5.
* ultralytics.data.augment.RandomPerspective.apply_bboxes() line 497-498 xy = np.ones((n * 4, 3), dtype=bboxes.dtype) may need 4 changed to 5. BUT DEBUGGER NOT EVEN HITTING THIS LINE. 

Need to get all transforms referenced by ultralytics.data.augment.v8_transforms, including:
  * pre-transforms including:
    - Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic),
    - CopyPaste(p=hyp.copy_paste),
    - # TODO: RandomPerspective() - TOO HARD RIGHT NOW
  * MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),
  * Albumentations(p=1.0),
  * RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
  * RandomFlip()

File "/home/david/production/ultralytics/ultralytics/utils/loss.py", line 757, in <listcomp>
    list_feat_parts = [xi.view(feats[0].shape[0], self.no, -1) for xi in feats]
                       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
RuntimeError: shape '[21, 99, -1]' is invalid for input of size 96600000
